		...
		if (mc.getFvect().size() > 0) {
			for (GroupVariable value : mc.getFvect()) {
				if (value.aggregate.equals("sum"))
					...
					writer.print("\t\t\t" + value.getString() + " = (" + value.getString() + "< newtuple."
							+ value.attribute + ") ? newtuple." + value.attribute + " : " + value.getString() + ";\n");
				else if (value.aggregate.equals("min"))
					writer.print("\t\t\t" + value.getString() + " = (" + value.getString() + "> newtuple."
							+ value.attribute + ") ? newtuple." + value.attribute + " : " + value.getString() + ";\n");
		...
		for (int i = 1; i <= mc.getNumber(); i++) {
			...
			for (GroupVariable value : mc.getFvect()) {
				if (Integer.parseInt(value.index) == i) {
					...
					if (value.aggregate.equals("sum"))
						writer.print(
								"\t\t\t\t\t\ttemp." + value.getString() + " += newtuple." + value.attribute + ";\n");
					if (value.aggregate.equals("max"))
						writer.print("\t\t\t\t\t\ttemp." + value.getString() + " = (temp." + value.getString()
								+ "< newtuple." + value.attribute + ") ? newtuple." + value.attribute + " :temp."
								+ value.getString() + ";\n");
					if (value.aggregate.equals("min"))
						writer.print("\t\t\t\t\ttemp." + value.getString() + " = (temp." + value.getString()
								+ "> newtuple." + value.attribute + ") ? newtuple." + value.attribute + " :temp."
	...
	private static void outputRequiredClass(PrintWriter writer, HashMap<String, String> dataType) {
		...
		for (GroupVariable temp : mc.getFvect()) {
			if (temp.aggregate.equals("avg")) {
				writer.print(
						"\t" + dataType.get(temp.attribute) + "\tsum_" + temp.attribute + "_" + temp.index + ";\n");
				writer.print(
						"\t" + dataType.get(temp.attribute) + "\tcount_" + temp.attribute + "_" + temp.index + ";\n");
			} else
